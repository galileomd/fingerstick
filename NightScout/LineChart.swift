...
class LineChart: UIControl {
	...
	var lowAlarm:Int = 100
	var highAlarm:Int = 250
    ...
    */
    func getMaximumValue() -> CGFloat {
		...
		if maximum < highAlarm
			{ maximum = highAlarm + 20}
	...
	*/
	...
	func scaleDataXAxis(data: [String]) -> Array<CGFloat> {
		var factor = drawingWidth / CGFloat(data.count - 1)
		var scaledDataXAxis: Array<CGFloat> = []
		for index in 0..<data.count {
			var newXValue = factor * CGFloat(index)
	...
	*/
	func drawAlarm(alarm: Int) {
		var maximumYValue = getMaximumValue()
		var height = drawingHeight / maximumYValue
		var context = UIGraphicsGetCurrentContext()
		var yHeight = self.bounds.height - (CGFloat(alarm) * height) - axisInset
		let dash:[CGFloat] = [2,10]
		
		CGContextSetLineDash(context, 0, dash, 2)
		CGContextSetLineWidth(context, 2)
		CGContextSetStrokeColorWithColor(context, UIColor.redColor().CGColor)
		CGContextMoveToPoint(context, axisInset, yHeight)
		CGContextAddLineToPoint(context, self.bounds.width - axisInset, yHeight)
		CGContextStrokePath(context)
	}
    ...
    */
    func drawXLabels() {
		...
		var scaledDataXAxis = xAxisLabels.isEmpty ? scaleDataXAxis(xAxisData) : scaleDataXAxis(xAxisLabels)
    ...
    */
    func drawYLabels() {
        ...
        for var index = 0; index <= Int(maximumYValue); index += step {
            ...
            var label = UILabel(frame: CGRect(x: -axisInset, y: yValue, width: axisInset, height: axisInset))
			label.font = UIFont.systemFontOfSize(10)
			label.textColor = UIColor.whiteColor()
	...
	* Add X Axis Label
	*/
	func addXAxisLabels(data: [String]) {
		for item in data
		{
